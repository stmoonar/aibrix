...
def check_label_args(args) -> None:
    ...
    fixed = (
        ...
        ("--fit-min-completed-requests",
         getattr(args, "fit_min_completed_requests", design.MIN_WINDOW_REQUESTS),
         design.MIN_WINDOW_REQUESTS),
    )
    wrong = [f"{flag}={value} (fixed by the design at {want})" for flag, value, want in fixed
             if float(value) != float(want)]
    align = getattr(args, "fit_window_align", design.WINDOW_ALIGN)
    if align != design.WINDOW_ALIGN:
...
class LadderRun:
    ...
    def __init__(self, args, model: str, priors: design.RhoPriors, *, factory,
        ...
        self.label = campaign.primary_label(args, model)
...
def run_ladder_campaign(args, *, drive: Optional[Callable] = None,
    ...
    # The fit plan of a ladder run selects its cells from this run's own ledger when the
    # fit runs (split / role / warm-up), so it can be written before the first cell.
    (out_dir / "fit_plan.json").write_text(
        json.dumps(campaign.fit_plan(models, out_dir, raw_dir, args, {},
                                     ledgers=[out_dir / LEDGER]), indent=2) + "\n",
        encoding="utf-8",
    )
    if args.dry_run:
        ...
        print(f"dry run: wrote {out_dir / 'plan.json'} and {out_dir / 'fit_plan.json'} "
              "(the run manifest is only written when a run starts)")
        return 0
