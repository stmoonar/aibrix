...
def publish_once(redis_client, *, node: str, ttl_s: int, collect=None) -> dict:
    collect = collect or collect_nvidia_smi
    payload = build_payload(node, collect())
...
def run_agent(
    ...
    *,
    ...
    while iterations is None or completed < iterations:
        try:
            publish_once(redis_client, node=node, ttl_s=ttl_s, collect=collect)
