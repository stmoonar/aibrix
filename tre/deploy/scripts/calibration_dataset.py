...
* ``windows.csv`` - all models, all non-void cells, one row per window, on the fitting
  windows (format revision 2: 30 s windows ending on the 10 s gateway grid, read
  ``(start, end]`` - the phase-aligned controller's window, plan 2026-09-21 §6.11 D8),
  labelled through :func:`scripts.rewindow_from_raw.label_cell` - the path the boundary
  search and the fit use - with the three arms of ``tre_common.slo_labels``:
  ``slo_label`` / ``slo_violated`` the primary D6' label (slowdown TTFT from the model's
  idle fit, >= 20 completed requests), ``slo_label_fixed`` the fixed 500 / 75 ms
  comparison and ``slo_label_k3`` the k = 3 / 150 ms ablation.
...
README = "DATASET.md"
#: 1: 30 s / 5 s free-phase windows, one fixed label (2026-09-23 main line).
#: 2: 30 s / 10 s grid-aligned windows (D8), three label arms (D6'), ttft_len_samples.
FORMAT_REVISION = 2
#: The fitting windows of revision 2 (plan 2026-09-21 §6.11 D8).
DEFAULT_WINDOW_MS = 30_000
DEFAULT_STEP_MS = 10_000
DEFAULT_WINDOW_ALIGN = rewindow_from_raw.WINDOW_ALIGN_GRID
...
class Settings:
    ...
    #: the parity check; None when the run recorded nothing usable.
    online_window_ms: Optional[int] = None
    ...
    def latency_slo_ms(self) -> dict:
        """The fixed thresholds (the ``slo_label_fixed`` arm)."""
...
def _settings_for(campaigns: Sequence[Path], overrides: dict) -> tuple[Settings, list[dict]]:
    ...
    for campaign in campaigns:
        ...
        online_align = online_align or prov.get("window_align")
        online_label = online_label or prov.get("label")
    ...
    settings = Settings(
        ...
        min_completed_requests=int(
            overrides.get("min_completed_requests") or slo_labels.DEFAULT_MIN_COMPLETED_REQUESTS),
        online_window_ms=None if window_ms is None else int(window_ms),
        online_step_ms=None if step_ms is None else int(step_ms),
        online_window_align=str(online_align or rewindow_from_raw.WINDOW_ALIGN_NONE),
...
def build_dataset(
    ...
    *,
    ...
    labels_by_model: dict[str, dict] = {}
    models_seen: list[str] = []
    fixed_min_n_diffs: dict[str, int] = {"windows_compared": 0, "differ": 0,
                                         "differ_all_low_n": 0}
    ...
    manifest = {
        ...
        "online_parity": {
            "what": (
                "each attempt re-windowed on the run's own online grid "
                f"({settings.online_window_ms} ms / {settings.online_step_ms} ms, align "
                f"{settings.online_window_align}) and labelled with the label definition the "
                "run recorded (the fixed arm); every window must equal the online CSV's "
                "slo_label"
            ),
            "online_label": settings.online_label,
            "fixed_arm_min_n_differences": fixed_min_n_diffs,
        },
        "probe_rule": {
...
def _convert_attempt(attempt, run_dir, settings, registry, plan_cells, discrepancies,
    ...
    if records and not void_reasons:
        ...
        if cell is not None:
            ...
            if attempt.primitive == gen.HOLD_PRIMITIVE and attempt.ledger is None:
                ...
                verdict = boundary.probe_verdict(kept, label=primary)
    ...
    if online_csv is not None and windows and status != STATUS_VOID:
        online_parity = _check_online_parity(
            attempt, online_csv, records, instants, guard, settings, registry, discrepancies,
            fixed_min_n_diffs,
        )
...
def online_label_definition(settings: Settings) -> Optional[slo_labels.LabelDefinition]:
    """The label the run's online CSVs were written with, rebuilt from its record.

    A 09-23 record (fixed thresholds, no ``min_n``) rebuilds as the fixed label with no
    min-n guard - exactly what labelled those CSVs."""
    if not settings.online_label:
        return None
    try:
        return slo_labels.LabelDefinition.from_dict(settings.online_label)
    except (KeyError, TypeError, ValueError):
        return None


def online_windows(records, instants, guard, cell, spec, settings: Settings, label) -> list[dict]:
    """One attempt re-windowed on the run's own online grid with ``label``."""
    return rewindow_from_raw.label_cell(
        records, instants, cell, spec,
        label=label,
        window_ms=int(settings.online_window_ms or settings.window_ms),
        step_ms=int(settings.online_step_ms or settings.step_ms),
        window_align=settings.online_window_align,
        percentile_mode=settings.percentile_mode,
        min_latency_samples=settings.min_latency_samples,
        instant_sample_interval_ms=SCRAPE_INTERVAL_MS,
        instant_grid=rewindow_from_raw.INSTANT_GRID_LIVE,
...
def compare_online(online_rows: Sequence[dict], ours: Sequence[dict], column: str) -> list[tuple]:
    """Windows (start, end, online, ours) where ``ours[column]`` differs from the online
    ``slo_label``; a window present on one side only is a difference too."""
    theirs = {(int(r["window_start_ms"]), int(r["window_end_ms"])): r[slo_labels.LABEL_COLUMN]
              for r in online_rows}
    mine = {(int(w["window_start_ms"]), int(w["window_end_ms"])): w.get(column) for w in ours}
    out = []
    for key in sorted(set(theirs) | set(mine)):
        if theirs.get(key) != mine.get(key):
            out.append((key[0], key[1], theirs.get(key), mine.get(key)))
    return out


def _check_online_parity(attempt, online_csv: Path, records, instants, guard, settings, registry,
                         discrepancies, fixed_min_n_diffs: Optional[dict] = None) -> str:
    """For a capture made with the shared labelling path, its online CSV must carry the
    very windows and labels this code recomputes on the run's own grid with the label the
    run recorded - a live check that the merged labelling did not change what the run's
    windows meant (plan: fixed arm, 30 s / 5 s). An older online CSV (server-side latency,
    30 s tumbling) cannot be compared and says so in the manifest; it is summarised once
    per run, not listed per cell.

    Also counted (``fixed_min_n_diffs``): windows where the dataset's own fixed arm - the
    same thresholds but with the D-line's min-n guard - differs from the online label;
    every such window should be one with fewer than min-n completions."""
    ...
    label = online_label_definition(settings)
    try:
        cell = r3_grid.GridCell.from_scenario_id(attempt.cell_id)
    except ValueError:
        return PARITY_NOT_COMPARABLE
    if label is None:
        return PARITY_NOT_COMPARABLE
    spec = registry.model(attempt.model)
    ours = online_windows(records, instants, guard, cell, spec, settings, label)
    diffs = compare_online(online, ours, slo_labels.LABEL_COLUMN)
    if fixed_min_n_diffs is not None:
        fixed = dataclasses_replace_min_n(label, settings.min_completed_requests)
        fixed_rows = online_windows(records, instants, guard, cell, spec, settings, fixed)
        by_key = {(int(w["window_start_ms"]), int(w["window_end_ms"])): w for w in fixed_rows}
        fixed_diffs = compare_online(online, fixed_rows, slo_labels.LABEL_COLUMN)
        fixed_min_n_diffs["windows_compared"] += len(online)
        fixed_min_n_diffs["differ"] += len(fixed_diffs)
        fixed_min_n_diffs["differ_all_low_n"] += sum(
            1 for start, end, _o, mine in fixed_diffs
            if mine == slo_labels.LABEL_UNLABELED
            and int((by_key.get((start, end)) or {}).get(slo_labels.COMPLETED_REQUESTS_COLUMN) or 0)
            < settings.min_completed_requests
        )
    if diffs:
        discrepancies.append(
            f"{attempt.stem}: online CSV windows/labels differ from the re-labelled raw in "
            f"{len(diffs)} window(s) ({len(online)} online vs {len(ours)} re-labelled)"
        )
        return PARITY_DIFFERENT
    return PARITY_IDENTICAL


def dataclasses_replace_min_n(label: slo_labels.LabelDefinition, min_n: int) -> slo_labels.LabelDefinition:
    return replace(label, min_completed_requests=int(min_n))
