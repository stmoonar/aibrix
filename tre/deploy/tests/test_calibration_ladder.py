...
def test_the_ladder_is_the_preregistered_table() -> None:
    ...
    # the label evidence floor and window grid are the D-line's (adopted 2026-09-23 over
    # the preregistration); the preregistered values are kept for the record
    assert design.MIN_WINDOW_REQUESTS == 20 and design.PREREGISTERED_MIN_WINDOW_REQUESTS == 10
    assert (design.STEP_MS, design.WINDOW_ALIGN) == (10_000, "grid")
    assert design.PREREGISTERED_STEP_MS == 5_000
...
def test_the_run_manifest_freezes_every_preregistered_parameter(tmp_path) -> None:
    ...
    assert pre["label"]["min_window_requests"] == 20
    assert (pre["label"]["step_ms"], pre["label"]["window_align"]) == (10000, "grid")
