...
def test_a_run_converts_into_the_standard_layout_without_touching_it(tmp_path):
    ...
    assert manifest["windowing"]["window_ms"] == 30000 and manifest["windowing"]["step_ms"] == 5000
...
def test_the_window_table_is_the_labelling_path_s_output(tmp_path):
    ...
    assert [(int(w["window_start_ms"]), w["slo_label"]) for w in ours] == [
        (t["window_start_ms"], t["slo_label"]) for t in theirs
    ]
