...
from scripts.admission_cap import ENGINE_CAPPED, GATEWAY_CAPPED, ramp_backlog_coefficient
...
def test_ramp_metadata_records_the_per_shape_timeline(capacity_model) -> None:
    ...
    expected = GATEWAY_CAPPED.ramp_seconds(c_s, gen.RAMP_BACKLOG_COEFFICIENT)
    ...
    assert meta["admission_cap"] == GATEWAY_CAPPED.name
...
def test_a_shape_whose_burst_would_be_shed_is_skipped(capacity_model) -> None:
    body, meta = gen.build_schedule("dsqwen-7b", "S1", "bursts", capacity_model, KV_7B)
    assert body is None
...
def test_build_schedule_from_capacity_rps_uses_the_supplied_capacity() -> None:
    ...
    expected = GATEWAY_CAPPED.ramp_seconds(20.0, gen.RAMP_BACKLOG_COEFFICIENT)
    assert meta["ramp_s"] == pytest.approx(expected, abs=1e-3)
    assert meta["drain_start_s"] == pytest.approx(expected * 1.25, abs=1e-3)
...
def test_generate_indexes_every_cell_and_writes_only_the_feasible_ones(
    ...
    cells = (len(gen.SHAPES) + 1) * len(gen.PRIMITIVES)
    ...
    # with this KV size S1 needs 576 requests and S4 314, both past the 240 admitted
...
def test_generating_for_the_engine_capped_policy_skips_nothing(tmp_path: Path) -> None:
    # The regression that proves the cap is a real parameter: move the binding limit into
    # the engine and every burst cell becomes observable, with a 300 s ramp everywhere.
