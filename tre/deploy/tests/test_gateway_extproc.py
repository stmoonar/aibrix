...
and an ORIGINAL_DST cluster. The tre-v2 rebuild must keep what the v2 experiments rely on:
...
* the same route timeout as the per-model HTTPRoutes (600 s), so switching paths does not
  silently change which requests time out.
"""
...
from gen_model_manifests import build_httproutes
...
EXTPROC_FILTER = f"envoy.filters.http.ext_proc/envoyextensionpolicy/tre-v2/{POLICY_NAME}/extproc/0"
...
def _one(kind: str) -> dict:
    (doc,) = [d for d in _docs(EXTPROC) if d["kind"] == kind]
    return doc
...
def _patches() -> list[dict]:
    ...
    return policy["spec"]["jsonPatches"]


def _clusters() -> dict[str, dict]:
    out = {}
    for patch in _patches():
        if patch["type"] == CLUSTER_TYPE:
            assert patch["operation"]["op"] == "add" and patch["operation"]["path"] == ""
...
def test_original_dst_clusters_carry_the_per_model_admission_limits() -> None:
    ...
    for name, cluster in _clusters().items():
        ...
        # Never Envoy's implicit default (1024 connections/requests shared per cluster).
...
def test_route_timeouts_equal_the_per_model_httproute_timeout() -> None:
    registry = load_registry(str(DEPLOY_ROOT / "registry.yaml"))
    route_timeouts = {
        rule["timeouts"]["request"] for route in build_httproutes(registry) for rule in route["spec"]["rules"]
    }
    assert route_timeouts == {"600s"}
    for route in _route_patches():
        assert route["route"]["timeout"] == "600s"
        assert route["route"]["idle_timeout"] == "3600s"


def test_extension_policy_matches_v1_request_path_and_is_never_the_binding_cap() -> None:
    ...
    assert ext["processingMode"]["request"] == {"body": "Buffered"}  # the plugin reads the model from the body
    assert "response" in ext["processingMode"]  # response headers carry target-pod back
    ...
    ext_cb = ext["backendSettings"]["circuitBreaker"]
    assert ext_cb["maxParallelRequests"] >= worst_case_streams
    assert ext_cb["maxPendingRequests"] >= worst_case_streams
...
def test_body_buffering_fits_long_prompts() -> None:
    ctp = _one("ClientTrafficPolicy")
    assert ctp["spec"]["targetRefs"][0]["name"] == "tre-aibrix-eg"
    assert ctp["spec"]["connection"]["bufferLimit"] >= 4 * 1024 * 1024
...
def test_plugin_deployment_gates_routing_on_the_routable_label() -> None:
    ...
    assert dep["spec"]["replicas"] == 1  # scrape loop is not leader-elected
    container = dep["spec"]["template"]["spec"]["containers"][0]
    env = {e["name"]: e.get("value") for e in container["env"]}
    assert env["TRE_ROUTABLE_LABEL_FILTER"] == "true"
    assert env["TRE_ROUTE_MODEL_HEADER"] == "true"
    assert env["HOT_SWITCH"] == "0"  # no gateway-initiated wake-ups in either arm
