...
#: (window alignment, step) pairs every parity test runs on: the 09-23 online grid and the
#: D8 fitting grid (30 s windows ending on the 10 s gateway grid, read (start, end]).
GRIDS = [("none", 5000), ("grid", 10000)]


def _args(window_align: str = "none", step_ms: int = 5000) -> Namespace:
    return Namespace(window_ms=30000, step_ms=step_ms, percentile_mode="bucket_upper",
                     min_latency_samples=10, window_align=window_align)


def _primary() -> slo_labels.LabelDefinition:
    """The primary (D6') label the probe is judged on: registry idle fit, 500 / 75 ms."""
    return slo_labels.label_def_for_model(
        MODEL, ttft_p95_ms=TTFT_SLO, tpot_p95_ms=TPOT_SLO, registry=str(REGISTRY_PATH))


def _online_rows(records, sidecar, spec, window_align: str = "none", step_ms: int = 5000) -> list[dict]:
    return r3_grid.label_schedule_cell_windows(
        _args(window_align, step_ms), spec, r3_grid.GridCell.from_scenario_id(CELL_ID), CELL_ID,
        records, sidecar,
        start_ms=START_MS, end_ms=START_MS + DURATION_S * 1000 + 400,
        truncated_at_ts_ms=None, label=_primary(),
    )
...
def _rewindow(monkeypatch, raw_dir: Path, out: Path, window_align: str = "none",
              step_ms: int = 5000) -> None:
    monkeypatch.setattr(sys, "argv", [
        "rewindow_from_raw.py", "--model", MODEL, "--raw-dir", str(raw_dir),
        "--output", str(out), "--window-ms", "30000", "--step-ms", str(step_ms),
        "--window-align", window_align,
...
@pytest.mark.parametrize("window_align,step_ms", GRIDS)
def test_the_probe_and_the_fit_label_the_same_capture_identically(
    tmp_path, monkeypatch, spec, window_align, step_ms,
):
    records, sidecar = _sender_records(), _sidecar()
    online = _online_rows(records, sidecar, spec, window_align, step_ms)
    ...
    _rewindow(monkeypatch, tmp_path / "raw", offline_csv, window_align, step_ms)

    # Same windows, same latency, same unserved counts, same labels - the same file.
    ...
    if window_align == "grid":
        assert all(int(r["window_end_ms"]) % 10000 == 0 for r in online)

    # ... and therefore the same probe verdict, from the in-memory rows and from the CSV
    # the campaign reads back - judged on the primary (D6') label the fit trains on.
    primary = _primary()
    assert (
        boundary.probe_verdict(online, label=primary)
        == boundary.probe_verdict(campaign.read_window_rows(online_csv), label=primary)
        == boundary.probe_verdict(campaign.read_window_rows(offline_csv), label=primary)
    )
...
def test_the_driver_itself_writes_what_the_re_window_reproduces(tmp_path, monkeypatch, spec):
    ...
    assert artifact["label"]["mode"] == "slowdown"  # the primary label, recorded
...
@pytest.mark.parametrize("window_align,step_ms", GRIDS)
def test_every_label_column_is_written_by_one_function(spec, window_align, step_ms):
    rows = _online_rows(_sender_records(), _sidecar(), spec, window_align, step_ms)
    arms = slo_labels.label_arms(_primary())
    for row in rows:
        expected = slo_labels.window_slo_label(row, arms["primary"])
        assert row["slo_label"] == expected
        assert row["slo_violated"] == (
            None if expected == "unlabeled" else expected == "violated"
        )
        assert row["slo_label_fixed"] == slo_labels.window_slo_label(row, arms["fixed_comparison"])
        assert row["slo_label_k3"] == slo_labels.window_slo_label(row, arms["ablation_k3_floor150"])
